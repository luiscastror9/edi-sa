...
namespace CNV_Site.Areas.BackOffice.Extensions
{
    public class EmailHelper
    {
        ...
        public string bodyhtml(CNV_Site.Areas.BackOffice.Consultas con, int opt = 0)
        {
            string body = string.Empty;
            //using streamreader for reading my htmltemplate   
            if (opt == 1)
            {
                using (StreamReader reader = new StreamReader(System.Web.Hosting.HostingEnvironment.MapPath("~/Views/Templates/contactoProteccion.html")))
                ...
                body = body.Replace("{Nombre}", con.NombreConsulta); //replacing the required things  
            ...
            else
            {
                ...
                body = body.Replace("{residencia}", con.residenciaConsulta);
