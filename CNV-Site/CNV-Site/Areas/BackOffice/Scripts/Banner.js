$(function () {
    ...
    $('#BannerData').DataTable({
        ...
        "autoWidth": false,
        "bServerSide": true,
        "sAjaxSource": "http://" + window.location.host + window.location.pathname.substring(0, window.location.pathname.lastIndexOf("/") + 1) + "/BackOffice/Default/BannerDataSurvey",
        "bProcessing": true,
        "language": {
        ...
        "aoColumns": [
            ...
            , {
                ...
                //"mRender": function (row) { return '<i class="fa fa-pencil edit-user-data data=' + row[0]+'" style="font- size: 22px;" data-original-title="Editar"></i>'; }
                "mRender": function (row) {
                    return '<img width="50px" height="50px" style="font- size: 22px;cursor: pointer;" src="http://' + window.location.host + window.location.pathname.substring(0, window.location.pathname.lastIndexOf("/") + 1) + '/Content/Upload/' + row['urlImgSlideHome'] + '"/>';
                }
    ...
    $('#BannerData tbody').on('click', '.edit-banner-data', function () {
        ...
        location.href = "http://" + window.location.host + window.location.pathname.substring(0, window.location.pathname.lastIndexOf("/") + 1) + "/BackOffice/Default/EditBanner/" + id;
    ...
    $('#BannerData tbody').on('click', '.delete-banner-data', function () {
        ...
        if (r == true) {
            $.ajax({
                url: "http://" + window.location.host + window.location.pathname.substring(0, window.location.pathname.lastIndexOf("/") + 1) + "/BackOffice/Default/DeleteBanner/" + id,
                type: "POST",
                dataType: 'HTML',
    ...
    $(document).on('click', '#btnNewBanner', function () {
        ...
        location.href = "http://" + window.location.host + window.location.pathname.substring(0, window.location.pathname.lastIndexOf("/") + 1) + "/BackOffice/Default/CreateBanner"
