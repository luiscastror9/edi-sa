$(function () {
    ...
    $('#ArticulosData').DataTable({
        ...
        "autoWidth": false,
        "bServerSide": true,
        "sAjaxSource": "http://" + window.location.host + window.location.pathname.substring(0, window.location.pathname.lastIndexOf("/") + 1) + "/BackOffice/Prensa/ArticulosDataSurvey",
        "bProcessing": true,
        "language": {
            ...
            { "mData": "titleNote", "title": "Descripcion", "sDefaultContent": "" },
             {
                ...
                "mRender": function (row) {
                    return '<img width="50px" height="50px" style="font- size: 22px;cursor: pointer;" src="http://' + window.location.host + window.location.pathname.substring(0, window.location.pathname.lastIndexOf("/") + 1) + '/Content/Upload/' + row['imgNote'] + '"/>';
                }
    ...
    $('#ArticulosData tbody').on('click', '.delete-Articulos-data', function () {
        ...
        if (r == true) {
            $.ajax({
                url: "http://" + window.location.host + window.location.pathname.substring(0, window.location.pathname.lastIndexOf("/") + 1) + "/BackOffice/Prensa/DeleteArticulos/" + id,
                type: "POST",
                dataType: 'HTML',
